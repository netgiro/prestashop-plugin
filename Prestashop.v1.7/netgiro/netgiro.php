...
use PrestaShop\PrestaShop\Core\Payment\PaymentOption;
	...
	class netgiro extends PaymentModule
	{
		...
		{
			if (!parent::install() || !$this->registerHook('invoice') || !$this->registerHook('paymentOptions') || !$this->registerHook('paymentReturn'))
			{
				return false;
			}
		...
		function hookPaymentOptions($params)
		{
			...
			$embeddedOption->setCallToActionText($this->l('Pay with Netgiro'))
				->setForm($this->context->smarty->fetch('module:netgiro/views/templates/hook/payment.tpl'));
		...
		function hookInvoice($params)
		{
			...
			return $this->context->smarty->fetch('module:netgiro/views/templates/hook/payment_return.tpl');
		}
		...
		function hookPaymentReturn($params)
		{
			...
			$query = "
			SELECT `netgiro_confirmation_code`, `netgiro_invoice_number` FROM `"._DB_PREFIX_."order_netgiro` WHERE `id_order` = \"".$params['order']->id."\"";
		...
		private function _getSettingsTabHtml()
		{
			...
			<div class="margin-form">
			    <label style="clear:both;">'.$this->l('Netgíró Gateway URL').'</label>
				...
				<br><br>
			    <label style="clear:both;">'.$this->l('Application ID').'</label>
				...
				<br><br>
			    <label style="clear:both;">'.$this->l('Secret Key').'</label>
				...
				<div style="display: none">
					<label style="clear:both;">'.$this->l('Allowed payment options').'</label>
					<input type="checkbox" name="netgiro_PaymentOption_1" id="netgiro_PaymentOption_1" style="vertical-align: middle;" value="1"  checked="checked" /> <label class="t" for="netgiro_PaymentOption_1">'.$this->l('14 days').'</label>
					<input type="checkbox" name="netgiro_PaymentOption_2" id="netgiro_PaymentOption_2" style="vertical-align: middle;" value="1" /> <label class="t" for="netgiro_PaymentOption_1">'.$this->l('Partial payments').'</label>
					<input type="checkbox" name="netgiro_PaymentOption_3" id="netgiro_PaymentOption_3" style="vertical-align: middle;" value="1" /> <label class="t" for="netgiro_PaymentOption_1">'.$this->l('Partial payments without interest').'</label>
					<br><br>
					<label style="clear:both;">'.$this->l('Maximum number of installments').'</label>
					<input style="width: 300px;" type="text" name="netgiro_MaxNumberOfInstallments" value="12" />
					<br><br>
				</div>
			    <label style="clear:both;">'.$this->l('IFrame').'</label>
